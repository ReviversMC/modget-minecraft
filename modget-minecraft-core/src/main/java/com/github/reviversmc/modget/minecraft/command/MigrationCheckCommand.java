...
public class MigrationCheckCommand extends CommandBase {
    private static final String COMMAND = "migrationcheck";
        ...
        CommandRegistrationCallback.EVENT.register((dispatcher, isDedicated) ->
            dispatcher.register(CommandManager.literal(Modget.NAMESPACE_SERVER)
                .then(CommandManager.literal(COMMAND)
                    .then(CommandManager.argument("minecraftVersion", StringArgumentType.greedyString())
                        .requires(source -> source.hasPermissionLevel(PERMISSION_LEVEL))
                        .executes(context -> {
                            PlayerEntity player = context.getSource().getPlayer();

                            new StartThread(player, StringArgumentType.getString(context, "minecraftVersion")).start();
                            return 1;
                        })
                    )
                )
    ...
    public void executeCommand(PlayerEntity player, String minecraftVersion) {
        ...
        if (errorMessageBuilder.length() != 0) {
            player.sendMessage(new TranslatableText("Errors occurred while searching for migratable mods:" + errorMessageBuilder.toString())
