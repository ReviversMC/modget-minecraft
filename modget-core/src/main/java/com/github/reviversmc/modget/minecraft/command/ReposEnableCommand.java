...
public class ReposEnableCommand extends CommandBase {
    private static final String[] COMMAND_PARTS = {"repos", "enable", "repoId"};
    ...
    public void executeCommand(PlayerEntity player, int repoId) throws NoSuchRepoException {
        ...
        try {
            repo = Modget.INSTANCE.REPO_MANAGER.getRepo(repoId);
        ...
        if (Modget.INSTANCE.REPO_MANAGER.getRepo(repoId).isEnabled()) {
            VersionAgnosticMessage.get().sendWarning(player, VersionAgnosticText.get().translatable(
                    String.format("warning.%s.repo_already_enabled", Modget.NAMESPACE)));
            return;
        }

        repo.setEnabled(true);
        VersionAgnosticMessage.get().sendInfo(player, VersionAgnosticText.get().translatable(
                String.format("commands.%s.repo_enabled", Modget.NAMESPACE),
                repoId,
                String.format("/%s %s",
                        ENVIRONMENT == EnvType.CLIENT ? Modget.NAMESPACE : Modget.NAMESPACE_SERVER,
